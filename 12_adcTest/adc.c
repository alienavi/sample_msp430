...
void main(void)
{
	...
	TACCR0 = 1023;								// Set Timer0 PWM Period
	TACCTL1 = OUTMOD_7;							// Set TA0.1 Waveform Mode
	...
	while(1)
	{
		...
		// Total time for 1 sample ??
		while(ADC10CTL1 & ADC10BUSY);			// Wait for conversion to end
		// setting duty cycle
		TACCR1 = ADC10MEM;						// Set Duty Cycle = ADC Value
	}
}
